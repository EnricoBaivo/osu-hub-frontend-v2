...
export function PrismaAdapter(
  ...
  return {
    ...
    createUser: ({ id: _id, ...data }) => {
      const promise = p.user.create({ data });
      return promise as Promise<AdapterUser>;
    },
    getUser: (id) => p.user.findUnique({ where: { id } }) as Promise<AdapterUser>,
    getUserByEmail: (email) => p.user.findUnique({ where: { email } }) as Promise<AdapterUser>,
