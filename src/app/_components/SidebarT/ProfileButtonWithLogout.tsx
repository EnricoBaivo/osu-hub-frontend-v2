...
const ProfileButtonWithLogout = ({
  ...
  return (
    <motion.div className="row container flex  h-20  max-w-full items-center text-white ">
      <AnimatePresence >
        {parentHover &&

          <motion.div initial={{ display: "none", width: "0%", opacity: 0 }} animate={{ width: "100%", display: "flex", transition: { delay: 0.25 }, opacity: 1 }} exit={{ width: "0%", opacity: 0 }} className="flex h-full w-full  flex-row items-center justify-between">

            <MotionLink animate={{ transition: { delay: 0.4 }, opacity: parentHover ? 1 : 0 }} target="_blank" className="w-72 h-20 gap-1 uppercase font-black my-12 hover:scale-110 transition-all flex flex-row items-center justify-around px-4 py-2 text-indigo-500 hover:text-yellow-500 bg-osuhub-dark-ice-blue border rounded-md backdrop-blur-2xl " href={env.NEXT_PUBLIC_DISCORD_INVITE_LINK}>
